...
// 1. Begin at start position.
// 2. Check distance to each neighbor.
//    if neighbor has null distance or a distance greater than the current cell's
//    distance +1, update that neighbor's distance value and make that cell the child
//    of the current cell and add that to a min priority queue.
//      - The min priority queue is a tree organized by the distance value.
//      - To find the next cell to move to with the lowest distance, start at the
//        root of the solver, then recursively retrieve nodes until one is found that is
//        unexplored.
