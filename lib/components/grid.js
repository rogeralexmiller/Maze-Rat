...
Grid.prototype.addCells = function(){
  ...
  for (var i = 0; i < 50 ; i++) {
    var row = [];
    for (var j = 0; j < 50; j++) {
      var displayRow = (i*10)+15;
      var displayCol = (j*10)+15;
      var cell = new Cell([displayRow, displayCol],[i,j], this);
      row.push(cell);
    }
    this.cells.push(row);
  }
...
Grid.prototype.clearChildren = function(){
  for (var i = 0; i < this.cells.length; i++) {
    for (var j = 0; j < this.cells.length; j++) {
      var cell = this.cells[i][j];
      cell.children = [];
...
Grid.prototype.validPath = function(cell){
